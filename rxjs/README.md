...
2. Всё должно собираться через Webpack (включая картинки и стили) и выкладываться на Github Pages через CI
...
Понятное дело, что по-хорошему, хорошо бы, чтобы с сервера приходили посты сразу со списком последних комментариев, но пока разработчики сервеной части упёрлись и сказали, что всё будет отдельно, а именно:
...
![](https://i.imgur.com/iGUX2pt.png)

Для выбора проекта:

![](https://i.imgur.com/uFC6Xlr.png)
